...
const GuestMenu = memo(function GuestMenu({ signIn, register }: PropsType) {
    ...
    <>
      <Menu.Item>
        ...
        >
          ...
          <div className="target">{signIn ?? "Sign in"}</div>
        </div>
      </Menu.Item>
      <Menu.Item>
        <div className="group">
          <Link
            href="/register"
            className="transition flex items-center text-blue-dark outline-none ring-transparent px-3.5 py-2 text-sm hover:bg-blue-main hover:text-yellow"
          >
            <ClipboardCheckIcon className="h-4 w-4 mr-2" />
            <div className="target">{register ?? "Register"}</div>
          </Link>
        </div>
      </Menu.Item>
    </>
